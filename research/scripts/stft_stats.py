...
def main() -> None:
    ...
    N_freqs = len(bands)
    P = np.zeros((
        N_subjects, N_channels, N_freqs, N_timesteps
    ))
    ...
    for f_i, eeg_filepath in tqdm(enumerate(rest_eeg_filepaths)):
        ...
        for ch_i, X_ch in enumerate(X_total):
            ...
            P[f_i, ch_i] = band_power

    variance_diag = np.var(P, axis=-1)  # axis=2 is time
    variance_mean = np.median(variance_diag, axis=1)  # shape: (N_subjects, N_freqs)

    fig = make_subplots(rows=5, cols=1, subplot_titles=band_names,
                        shared_yaxes=True)
    ...
    for freq_idx in range(N_freqs):
        ...
        jitter_strength = 0.1
        x_jittered = [freq_idx + random.uniform(-jitter_strength, jitter_strength) for _ in range(N_subjects)]
